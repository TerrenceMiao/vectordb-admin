import { Container, List, ScrollArea, Tabs, Text, rem } from "@mantine/core";
...
const RecordDetailModal = ({
  innerProps,
}: ContextModalProps<{ record: Record }>) => {
  ...
  return (
    <Tabs defaultValue="Document">
      <Tabs.List grow>
        <Tabs.Tab
          value="Document"
          leftSection={<IconBlockquote style={iconStyle} />}
        >
          Document
        </Tabs.Tab>
        ...
        <Tabs.Tab
          value="Metadata"
          leftSection={<IconBraces style={iconStyle} />}
        >
          Metadata
        </Tabs.Tab>
      </Tabs.List>
      ...
      <Tabs.Panel value="Metadata">
        <Container p={"md"}>
          <ScrollArea h={250}>
            <Text size={"sm"}>
              {JSON.stringify(innerProps.record.metadata)}
            </Text>
          </ScrollArea>
        </Container>
      </Tabs.Panel>
